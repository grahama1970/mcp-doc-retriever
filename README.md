...
*   ✅ **Organized Storage:** Saves Git clones preserving the repository structure (`repo/`); saves web files in a flat structure per host using hashed filenames (`<host>/<url_base>-<hash>.<ext>`) within a persistent volume. *(Handled by `downloader` package & helpers)*
