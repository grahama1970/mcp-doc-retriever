...
*   ✅ **Download Indexing:** Maintains a JSON Lines index file per download job (`<download_id>.jsonl`), mapping original URLs/files to canonical URLs/paths, the generated local file path, content MD5 hashes, and detailed fetch status. *(Generated by `downloader.web`, used by `searcher`)*
    ...
    subgraph "Docker Container: mcp-doc-retriever"
        ...
        subgraph "Download Flow (Background Task)"
            ...
            subgraph "Web Source (Website/Playwright)"
                ...
                WebDownloader -- "Calc Path" --> HelperFuncs("🛠️ Helpers - downloader/helpers.py")
...
└── src/
    └── mcp_doc_retriever/ # Main application source code
        ...
        │   ├── git_downloader.py # Git clone/scan logic (renamed from git.py)
        │   ├── web_downloader.py # Web crawling logic (renamed from web.py)
