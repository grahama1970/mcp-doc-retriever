...
def extract_content_blocks_from_markdown(
    ...
    """Extracts structured content blocks (code, json, text) from Markdown."""
    content_blocks: List[ContentBlock] = []
    if not md_content:
        return content_blocks
    ...
    used_line_spans: Set[Tuple[int, int]] = set()
    # Keep track of character indices covered by code blocks to extract text later
    processed_indices = set()

    # Regex for fenced code blocks (e.g., ```python ... ```)
    # Allows optional language identifier
    fenced_code_regex = re.compile(
        # Start of line, optional spaces/tabs, three backticks, optional language, optional whitespace, newline
        r"^[ \t]*```(?P<lang>\w+)?\s*\n"
        # Capture the code content (non-greedy)
        r"(?P<code>.*?)\n"
        # Start of line, optional spaces/tabs, three backticks, optional trailing whitespace
        r"^[ \t]*```[ \t]*$",
        re.MULTILINE | re.DOTALL,  # Multiline for ^$, Dotall for .*? to match newlines
    )

    # Regex for indented code blocks (4 spaces or 1 tab)
    # Looks for blocks preceded by blank lines or start of document/line
    indented_code_regex = re.compile(
        # Positive lookbehind for two newlines OR start of line after newline OR start of document
        r"(?:(?<=\n\n)|(?<=^\n)|^)"
        # Capture one or more lines starting with 4 spaces or a tab
        r"((?:(?:[ ]{4}|\t).*\n?)+)"
    )

    # 1. Process Fenced Code Blocks first (usually more reliable)
    for match in fenced_code_regex.finditer(md_content):
        start_char, end_char = match.span()  # Get character indices
        # Skip if this block overlaps with an already processed one (unlikely but possible with regex)
        if any(i in processed_indices for i in range(start_char, end_char)):
            continue

        lang = match.group("lang")  # Extracted language (e.g., 'python') or None
        code = match.group("code").strip("\n")  # The actual code content
        if not code:
            continue  # Skip empty code blocks

        # Check if it's JSON
        # Requires language hint 'json' and valid parsing
        is_json = lang and lang.lower() == "json" and _is_json_like(code)
        content_type = "json" if is_json else "code"
        final_language = "json" if is_json else lang  # Override lang if JSON

        # Find line numbers
        start_line, end_line = _find_block_lines(code, source_lines, used_line_spans)

        parsed_json_data = None
        metadata = {"selector": f"fenced_{lang or 'code'}"}
        if is_json:
            try:
                parsed_json_data = json.loads(code)
                metadata["parsed_json"] = parsed_json_data
            except json.JSONDecodeError:
                logger.warning(
                    f"Markdown fenced block tagged 'json' failed parsing. Treating as code. URL: {source_url}"
                )
                content_type, final_language = "code", lang
        ...
        content_blocks.append(
            ContentBlock(
                ...
                content=code,
                language=final_language,
                block_type="fenced",  # Indicate it came from a fenced block
                start_line=start_line,
                end_line=end_line,
                source_url=source_url,
                metadata=metadata,
        ...
        # Mark these character indices as processed
        processed_indices.update(range(start_char, end_char))

    # 2. Process Indented Code Blocks
    # Use search with offset to find all occurrences
    offset = 0
    while offset < len(md_content):
        match = indented_code_regex.search(md_content, offset)
        if not match:
            break  # No more matches found

        # Use span(1) to get indices of the captured group (the indented lines)
        start_char, end_char = match.span(1)
        offset = end_char  # Start next search after this block

        # Skip if this block overlaps with an already processed one (e.g., fenced block)
        if any(i in processed_indices for i in range(start_char, end_char)):
            continue

        indented_block_text = match.group(1)
        # Remove the indentation (4 spaces or 1 tab) from each line
        code_lines = [
            line[4:] if line.startswith("    ") else line.lstrip("\t")
            for line in indented_block_text.splitlines()
        ]
        code = "\n".join(code_lines)
        if not code.strip():
            continue  # Skip if block becomes empty after de-indenting

        # Check if it's JSON (less likely for indented, but possible)
        is_json = _is_json_like(code)
        content_type = "json" if is_json else "code"
        final_language = (
            "json" if is_json else None
        )  # Indented blocks don't have lang hints

        # Find line numbers
        start_line, end_line = _find_block_lines(code, source_lines, used_line_spans)

        parsed_json_data = None
        metadata = {"selector": "indented_code"}
        if is_json:
            try:
                parsed_json_data = json.loads(code)
                metadata["parsed_json"] = parsed_json_data
                ...
                logger.warning(
                    f"Markdown indented block looked like JSON but failed parsing. Treating as code. URL: {source_url}"
                )
                content_type, final_language = "code", None
        ...
        content_blocks.append(
            ContentBlock(
                ...
                content=code,
                language=final_language,
                block_type="indented",  # Indicate it came from indentation
                start_line=start_line,
                end_line=end_line,
                source_url=source_url,
                metadata=metadata,
    ...
    # 3. Extract Text Blocks (everything not marked as processed code)
    text_segments = []
    current_segment_start = 0
    # Sort the processed indices to handle them in order
    sorted_processed = sorted(list(processed_indices))
    # Iterate through processed spans to find text gaps between them
    for i in range(len(sorted_processed)):
        proc_index = sorted_processed[i]
        # If there's a gap between the end of the last segment and the start of this processed block
        if proc_index > current_segment_start:
            # Extract the text in the gap
            text_segments.append(md_content[current_segment_start:proc_index])
        # Update the start for the next potential gap to be after this processed block
        # Need to find the end of the current contiguous processed block
        current_segment_end = proc_index
        while (
            i + 1 < len(sorted_processed)
            and sorted_processed[i + 1] == current_segment_end + 1
        ):
            i += 1
            current_segment_end = sorted_processed[i]
        current_segment_start = current_segment_end + 1

    # Add any remaining text after the last processed block
    if current_segment_start < len(md_content):
        text_segments.append(md_content[current_segment_start:])

    # Join all text segments and process them
    full_text_content = "".join(text_segments)

    # Split the combined text into paragraphs based on blank lines
    # Also handle potential single newlines within paragraphs if needed
    # Using a simple split on two or more newlines (common Markdown paragraph separator)
    for paragraph in re.split(r"\n\s*\n+", full_text_content):
        text = paragraph.strip()
        # Simple cleaning: remove potential Markdown heading markers like ###
        text = re.sub(r"^[#]+\s+", "", text).strip()
        # Basic list item marker removal (example)
        text = re.sub(r"^\s*[-*+]\s+", "", text).strip()
        # Add more cleaning rules if needed (e.g., for bold/italic markers)

        if text:  # Only add non-empty paragraphs
            # Find line numbers for the text paragraph
            start_line, end_line = _find_block_lines(
                text, source_lines, used_line_spans
            )
            content_blocks.append(
                ContentBlock(
                    type="text",
                    content=text,
                    block_type="text",  # General text block from Markdown
                    start_line=start_line,
                    end_line=end_line,
                    source_url=source_url,
                    metadata={"selector": "paragraph"},  # Approximate selector
                )
            )

    logger.debug(f"Extracted {len(content_blocks)} MD blocks from {source_url}")
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        sample_md = "# Header\n\nPara1 with *emphasis*.\n\n```py\ndef greet():\n    print('hello')\n```\n\nAnother para.\n\n    Indented code\n    More indent"
        md_blocks = extract_content_blocks_from_markdown(sample_md, "http://ex.com/m")
        print(f"Extracted MD Blocks ({len(md_blocks)}):")
        for b in md_blocks:
            lang = getattr(b, "language", None)
            print(
                f"  - Type:{b.type}, Lang:{lang}, Start:{b.start_line}, Content:'{b.content[:20]}'..."
            )
        assert len(md_blocks) > 0, "Test Failed: Should extract MD blocks"
