...
Description: This module extracts code blocks and descriptions from reStructuredText (.rst) files using the `docutils` library and integrates with tree-sitter for code metadata.
"""
...
from mcp_doc_retriever.context7.sparse_checkout import (
    sparse_checkout,
        ...
        for node in document.traverse():
            ...
            if isinstance(node, nodes.literal_block):
                ...
                if isinstance(
                    ...
                    logger.info("Found a code-block directive block.")
                    process_extracted_code(
                        extracted_data, node, file_path, repo_link, "code-block"
                    )

                # Check for 'testcode' directive
                elif isinstance(
                    node.parent, nodes.container
                ) and "testcode" in node.parent.get("classes", []):
                    logger.info("Found a testcode directive block.")
                    process_extracted_code(
                        extracted_data, node, file_path, repo_link, "testcode"
                    )

                # Check for 'literalinclude' directive - Requires different handling as it includes a file
                elif isinstance(
                    node.parent, nodes.container
                ) and "literalinclude" in node.parent.get("classes", []):
                    logger.info("Found a literalinclude directive block.")
                    # For literalinclude, you'll need to resolve the file path and read the content
                    try:
                        # Extract the file name from the directive's options
                        file_name = node.get(
                            "filename", None
                        )  # Adjust based on how the filename is stored
                        if file_name:
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Error reading included file: {e}", exc_info=True
                                )
                        else:
                            logger.warning(
                                "Filename not found in literalinclude directive."
                            )

                    except Exception as e:
                        logger.error(
                            f"Error processing literalinclude block: {e}", exc_info=True
                        )
...
def process_extracted_code(
    extracted_data, node, file_path, repo_link, code_type, code_block=None
):
    """
    Processes extracted code block, factoring out common operations.
    """
    try:
        ...
        description = get_description(node)  # Use helper function
        ...
        logger.error(f"Error processing {code_type} block: {e}", exc_info=True)
...
def usage_function():
    ...
    logger.add(sys.stderr, level="DEBUG", colorize=True)  # Add a handler for the terminal at DEBUG level
    
    repo_url = "https://github.com/arangodb/python-arango.git"
    repo_dir = "/tmp/python_arango_sparse"
    repo_link = "https://github.com/arangodb/python-arango/blob/main/docs/aql.rst"  # Correct and specific repo link
    exclude_patterns = []
    patterns = ["docs/*"]  # Sparse checkout pattern to get the docs directory
